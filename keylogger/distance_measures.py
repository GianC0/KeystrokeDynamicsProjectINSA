...
def produce_merged_model(data, user):
    if len(data[user]) == 0:
        print("ERROR: model data is empty > Results are wrong")
        raise
...
def get_eer(distance_user, distance_intruder):
    ...
    fpr, tpr, threshold = metrics.roc_curve(
        y_true=labels, y_score=distances, drop_intermediate=False
    )
    fnr = 1 - tpr
        ...
        for metric in data[user_d]:
            if metric != "raw_data":
                ...
                distance = get_distance(
                    models[metric], data[user_d][metric], "manhattan"
                )
                if user_d == user:
                    distances_user[metric] = distance
                else:
                    distances_intruder[metric] = np.concatenate(
                        [distances_intruder.setdefault(metric, []), distance]
                    )
...
def merge_data_with_split(data, split_off=0, random_split=False, metrics_tu=None):
    ...
    for user in data:
        ...
        for m_tu in metrics_tu:
            if not merged_metrics:
                ...
                merged_metrics = list(
                    map(np.concatenate, zip(merged_metrics, data[user][m_tu]))
                )
        if random_split:
    ...
    for user in merged_userdata:
        ...
        if o_size != len(merged_testdata[user]) + len(merged_userdata[user]):
            print(
                "ERROR: Splitting merged data resulted in length mismatch > Check dimensions of used data"
            )
...
def estimate_single_user(models, entry, d_measure, intruder_threshold):
    distances = {}
    for user in models:
        distances[user] = get_distance(
            models[user], entry.reshape(1, len(entry)), d_measure
        )
...
def get_user(
    ...
    model_data, test_data = merge_data_with_split(
        data_unmerged, split_off, random_split, metrics_tu
    )
    ...
    for user in test_data:
        for entry in test_data[user]:
            est_user, min_distance = estimate_single_user(
                models, entry, d_measure, intruder_threshold
            )
    ...
    return (
        correct_user_est,
        false_user_est,
        false_pos_intruder_est,
        correct_intruder_est,
        false_pos_users,
        mean_user_dist,
        mean_intruder_dist,
    )
...
def get_user_online(
    entry, intruder_threshold=100000, d_measure="manhattan", metrics_tu=None
):
    if metrics_tu is None:
        metrics_tu = ["hold_time"]
    data = get_processed_data()
    data.pop("Russian_or_Chinese_hacker")
    model_data, _ = merge_data_with_split(
        data, split_off=0, random_split=False, metrics_tu=metrics_tu
    )
    models = {}
    for user in model_data:
        models[user] = produce_merged_model(model_data, user)
...
if __name__ == "__main__":
    ...
    if to_run == "unified":
        ...
        (
            ...
            intruder_data,
            nmbr_test_data,
            random_test_sampling,
            metrics_to_use,
            distance_measure,
            intruder_distance_threshold,
        )
        print(
            "Correct user identification: {} out of {}".format(
                correct_user_est, nmbr_test_data * len(u_data.keys())
            )
        )
        print(
            "False user identifications: {}; From these: {} were mislabeled as intruders".format(
                false_user_est, false_pos_intruder_est
            )
        )
